...
from model import CodeBERTModel, GraphCodeBERTModel, UniXcoderModel, QwenSemanticModel
...
class EmbedQwen(Embed):
    ...
    def __init__(self, model_name, finetuned_model_path=None):
        ...
        self.code_length = 256
        self.data_flow_length = 0
        self.batch_size = 16
        self.flag = 1
        self.finetuned_model_path = finetuned_model_path

    def load_pretrained_model(self):
        self.model = QwenSemanticModel(
            self.model,
    ...
    def load_finetuned_model(self):
        ...
        if self.finetuned_model_path and os.path.exists(self.finetuned_model_path):
            self.model.load_state_dict(
                torch.load(self.finetuned_model_path, map_location=self.device),
                strict=False
            )
    ...
    def extract_corpus_vecs(self, methods):
        ...
        for batch in corpus_dataloader:
            ...
            with torch.no_grad():
                corpus_vec = self.model(code_inputs=corpus_inputs)
                corpus_vecs.append(corpus_vec.cpu().numpy())
