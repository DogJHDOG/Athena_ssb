...
def DFG_javascript(root_node,index_to_code,states):
    ...
    if (len(root_node.children)==0 or root_node.type=='string') and root_node.type!='comment':
        ...
        return sorted(DFG,key=lambda x:x[1])


# Targets the fwcd/tree-sitter-kotlin grammar. Node names that differ between
# kotlin-grammar forks fall through to the default recursive branch.
def DFG_kotlin(root_node,index_to_code,states):
    assignment=['assignment']
    def_statement=['property_declaration']
    ...
    if (len(root_node.children)==0 or root_node.type=='string') and root_node.type!='comment':
        ...
        if root_node.type==code:
            ...
            if root_node.type in ('identifier','simple_identifier'):
                states[code]=[idx]
            return [(code,idx,'comesFrom',[],[])],states
    elif root_node.type in def_statement:
        # property_declaration: [modifiers? ('val'|'var') variable_declaration (':' type)? ('=' expression)?]
        name=None
        value=None
        seen_eq=False
        for child in root_node.children:
            if child.type=='variable_declaration' and name is None:
                name=child
            elif child.type=='=':
                seen_eq=True
        ...
        if value is None:
            ...
            name_indexs=tree_to_variable_index(name,index_to_code)
            value_indexs=tree_to_variable_index(value,index_to_code)
            temp,states=DFG_kotlin(value,index_to_code,states)
            DFG+=temp
            for index1 in name_indexs:
                idx1,code1=index_to_code[index1]
                for index2 in value_indexs:
        ...
        # Kotlin assignment: [directly_assignable_expression ('='|assignment_and_operator) expression]
        ...
        right_nodes=root_node.children[-1]
        DFG=[]
        temp,states=DFG_kotlin(right_nodes,index_to_code,states)
        DFG+=temp
        name_indexs=tree_to_variable_index(left_nodes,index_to_code)
        value_indexs=tree_to_variable_index(right_nodes,index_to_code)
        for index1 in name_indexs:
        ...
        for child in root_node.children:
            ...
            if not seen_else:
                ...
                temp,else_states=DFG_kotlin(child,index_to_code,states)
                DFG+=temp
                branch_states.append(else_states)
        branch_states.append(current_states)
        if not has_else:
        ...
        for key in new_states:
            new_states[key]=sorted(list(set(new_states[key])))
        return sorted(DFG,key=lambda x:x[1]),new_states
    elif root_node.type in when_statement:
        # when_expression: each when_entry is a parallel branch
        DFG=[]
        branch_states=[]
        for child in root_node.children:
            if child.type=='when_entry':
                temp,entry_states=DFG_kotlin(child,index_to_code,states.copy())
                DFG+=temp
                branch_states.append(entry_states)
            else:
                temp,states=DFG_kotlin(child,index_to_code,states)
                DFG+=temp
        ...
        # Kotlin for: for (variable_declaration in expression) body — enhanced_for-like
        name=None
        value=None
        body=None
        seen_in=False
        for child in root_node.children:
            if child.type=='variable_declaration' and name is None:
                ...
                seen_in=True
            elif child.type=='control_structure_body' and body is None:
                body=child
            elif seen_in and value is None and child.type not in (')','in'):
                value=child
        DFG=[]
        if name is None or value is None or body is None:
            for child in root_node.children:
                temp,states=DFG_kotlin(child,index_to_code,states)
                DFG+=temp
            return sorted(DFG,key=lambda x:x[1]),states
        for i in range(2):
            ...
            for index1 in name_indexs:
                ...
                for index2 in value_indexs:
                    ...
                    DFG.append((code1,idx1,'computedFrom',[code2],[idx2]))
                states[code1]=[idx1]
            temp,states=DFG_kotlin(body,index_to_code,states)
            DFG+=temp
        dic={}
        for x in DFG:
            if (x[0],x[1],x[2]) not in dic:
        ...
        return sorted(DFG,key=lambda x:x[1]),states,states
